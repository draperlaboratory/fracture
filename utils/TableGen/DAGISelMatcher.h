...
void EmitMatcherTable(const Matcher *Matcher, const CodeInvDAGPatterns &CIP,
                      raw_ostream &OS);
...
class CheckOpcodeMatcher : public Matcher {
  const SDNodeInfo *SDOpcode;
  const CodeGenInstruction *TgtOpcode;
  bool isSDNode;
public:
  CheckOpcodeMatcher(const SDNodeInfo &opcode)
    : Matcher(CheckOpcode), SDOpcode(&opcode), TgtOpcode(0), isSDNode(true) {}

  CheckOpcodeMatcher(const CodeGenInstruction &opcode)
...
public:
  MorphNodeToMatcher(const std::string &opcodeName,
                     ...
                     bool hasChain, bool hasInFlag, bool hasOutFlag,
                     bool hasmemrefs,
                     int numfixedarityoperands, const InvPatternToMatch &pattern)
    : EmitNodeMatcherCommon(opcodeName, vts, numvts, operands, numops, hasChain,
                            hasInFlag, hasOutFlag, hasmemrefs,
  ...
  const InvPatternToMatch &getPattern() const { return Pattern; }
